...
/*需要绑定的数据*/
  ...
  public View getView(int position, View convertView, ViewGroup parent) { 
	  ...
	  /*设置各个控件的展示内容*/
	  holder.tv_barcode.setText("图书条形码：" + mData.get(position).get("barcode").toString());
	  holder.tv_bookType.setText("图书类别：" + (new BookTypeService()).GetBookType(Integer.parseInt(mData.get(position).get("bookType").toString())).getBookTypeName());
	  holder.tv_bookName.setText("图书名称：" + mData.get(position).get("bookName").toString());
