...
/*需要绑定的数据*/
  ...
  public View getView(int position, View convertView, ViewGroup parent) { 
	  ...
	  /*设置各个控件的展示内容*/
	  holder.tv_book.setText("图书：" + (new BookService()).GetBook(mData.get(position).get("book").toString()).getBookName());
	  holder.tv_reader.setText("读者：" + (new ReaderService()).GetReader(mData.get(position).get("reader").toString()).getReaderName());
	  try {holder.tv_borrowDate.setText("借阅日期：" + mData.get(position).get("borrowDate").toString().substring(0, 10));} catch(Exception ex){}
	  try {holder.tv_returnDate.setText("归还日期：" + mData.get(position).get("returnDate").toString().substring(0, 10));} catch(Exception ex){}
	  /*返回修改好的view*/
	  return convertView; 
    } 
